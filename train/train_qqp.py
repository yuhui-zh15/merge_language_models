...
def train(model_name: str, pretrained: bool, number_epochs: int):
    ...
    def preprocess_function(examples, verbose: bool = False):
        processed_sentences = []
        for i in range(len(examples["question1"])):
            sentence_1 = examples["question1"][i].strip()
            sentence_2 = examples["question2"][i].strip()
            ...
            processed_sentence = f"The semantic meanings of questions '{sentence_1}' and '{sentence_2}' are {'duplicate' if label == 1 else 'not_duplicate'}."
    ...
    training_args = TrainingArguments(
        output_dir=f"dumps/finetuned_{model_name}_pretrained{pretrained}_qqp_epochs{number_epochs}",
    ...
    torch.save(model.state_dict(), 'qqp_params.pth')
